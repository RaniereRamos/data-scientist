...
## TIDYR
# Remodelagem de dados / Reshaping data
...
df <- data.frame (
   nome = c("Game Of Thrones", "Mr. Robot", "Sillicon Valley"),
   regiaoA = c(97, 80, 84), 
   regiaoB = c(86, 90, 91))
...
# Criando dados / Creating data part2
set.seed(10)
df2 <- data.frame(
   id <- 1:4,
...
df2_organizado2 <- df2_organizado1 %>%
   separate(key, into = c("localidade", "tempo"))

df2_organizado2 %>% head(8)

# Mais um exemplo / More an example
set.seed(1)
df3 <- data.frame(
...
df3 %>%
   ...
   facet_wrap(~ group) + geom_smooth(method = "lm", aes(group = 1), se = F)
